...
class PatchTour(BaseModel):
    price: int = Field(ge=500, lt=3000, examples=[500, 2500])
    title: str = Field(
        min_length=3, examples=["Єгипет.Хургада"]
    )

class NewTour(PatchTour):
    ...
    tourist: int = Field(ge=1, lt=5, examples=[2])
    tate_of_dispatch: str = Field(min_length=3, max_length=500, examples=['12 березня'])
    tour_duration: str = Field(ge=1, lt=30, examples=[7])
